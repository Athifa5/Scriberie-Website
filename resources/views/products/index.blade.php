...
@section('contents')
    <div class="d-flex align-items-center justify-content-between">
        <h1 class="mb-0">List Of Notes</h1>
        <a href="{{ route('products.create') }}" class="btn btn-primary">Add Note</a>
    </div>
    <hr />
            ...
            @if($product->count() > 0)
                @foreach($product as $rs)
                    <tr>
                        ...
                        <td class="align-middle">{{ $rs->type_of_notes }}</td>
                        <td class="align-middle">{{ $rs->important }}</td>  
